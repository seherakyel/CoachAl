...
const _NAV = [
    ...
    { id: "cv-analysis", icon: "description",        label: "CV Analysis", href: "cv-analysis.html" },
    { id: "interviews",  icon: "record_voice_over",  label: "Interviews",  href: "interviews.html" },
    { id: "reports",     icon: "analytics",          label: "Reports",     href: "reports.html" },
...
<div><div class="text-xl font-bold text-slate-900 leading-tight">CoachAI</div><div class="text-xs text-slate-500">AI Interview Coach</div></div>
</div>
<nav class="flex-1 px-4 space-y-1">${_NAV.map(i => _navLink(i, active)).join("")}</nav>
<div class="px-4 mt-auto space-y-3">
<div class="bg-surface-container rounded-lg p-4"><p class="text-xs text-on-surface-variant mb-3">Unlock advanced features</p>
<button class="w-full bg-primary text-on-primary text-sm py-2 px-4 rounded-lg hover:opacity-90 transition-opacity">Upgrade to Pro</button></div>
<a href="#" class="flex items-center gap-3 px-3 py-2 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-50 text-sm font-medium transition-colors"><span class="material-symbols-outlined text-[20px]">help</span>Support</a>
<button id="ui-signout" class="flex w-full items-center gap-3 px-3 py-2 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-50 text-sm font-medium transition-colors"><span class="material-symbols-outlined text-[20px]">logout</span>Sign Out</button>
</div></aside>`;
}
...
<input class="w-full pl-10 pr-4 py-1.5 bg-surface-container-low border border-slate-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-colors" placeholder="Search..."/></div></div>
